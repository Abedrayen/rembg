...
def np_to_torch(x, permute=True):
    # Convert the NumPy array to a torch tensor and cast to float on the desired device.
    tensor = torch.from_numpy(x).float().to(device)
    
    # If the input is 2D (shape: H x W), add a channel dimension to get shape H x W x 1.
    if tensor.dim() == 2:
        tensor = tensor.unsqueeze(-1)
    
    # If permute is True and the tensor has three dimensions (H, W, C),
    # permute it to channel-first ordering (C, H, W).
    if permute and tensor.dim() == 3:
        tensor = tensor.permute(2, 0, 1)
    
    # Finally, add a batch dimension at the front so that the output shape is (1, C, H, W).
